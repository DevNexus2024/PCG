...
function displayMenuItems(items) {
    ...
    items.forEach(item => {
        ...
        let imageUrl = item.imageUrl && item.imageUrl.trim() !== '' ? item.imageUrl : fallbackImage;
        ...
        if (imageFile) {
            ...
            while (!uploadSuccess && uploadAttempts < maxAttempts) {
                try {
                    ...
                    const storageRef = storage.ref('menuItems/' + timestamp + '_' + sanitizedName);
                    ...
                    if (uploadAttempts >= maxAttempts) {
                        alert('Failed to upload image after ' + maxAttempts + ' attempts. The menu item will be saved without an image.\n\nError: ' + uploadError.message);
                        imageUrl = null;
                    } else {
                        // Wait before retry
